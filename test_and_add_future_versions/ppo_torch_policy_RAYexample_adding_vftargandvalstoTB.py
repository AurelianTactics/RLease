...
class PPOTorchPolicy(TorchPolicy, LearningRateSchedule, EntropyCoeffSchedule):
    ...
    def loss(
        ...
        # added
        model.tower_stats['mean_vf_target'] = torch.mean(train_batch[Postprocessing.VALUE_TARGETS])
        model.tower_stats['min_vf_target'] = torch.min(train_batch[Postprocessing.VALUE_TARGETS])
        model.tower_stats['max_vf_target'] = torch.max(train_batch[Postprocessing.VALUE_TARGETS])
        model.tower_stats['std_vf_target'] = torch.std(train_batch[Postprocessing.VALUE_TARGETS])
        model.tower_stats['mean_vf_value'] = torch.mean(model.value_function())
        model.tower_stats['min_vf_value'] = torch.min(model.value_function())
        model.tower_stats['max_vf_value'] = torch.max(model.value_function())
        model.tower_stats['std_vf_value'] = torch.std(model.value_function())
        ...
        return convert_to_numpy(
            {
                ...
                # added
                "mean_vf_target": torch.mean(
                    torch.stack(self.get_tower_stats("mean_vf_target"))
                ),
                "min_vf_target": torch.mean(
                    torch.stack(self.get_tower_stats("min_vf_target"))
                ),
                "max_vf_target": torch.mean(
                    torch.stack(self.get_tower_stats("max_vf_target"))
                ),
                "std_vf_target": torch.mean(
                    torch.stack(self.get_tower_stats("std_vf_target"))
                ),
                "mean_vf_value": torch.mean(
                    torch.stack(self.get_tower_stats("mean_vf_value"))
                ),
                "min_vf_value": torch.mean(
                    torch.stack(self.get_tower_stats("min_vf_value"))
                ),
                "max_vf_value": torch.mean(
                    torch.stack(self.get_tower_stats("max_vf_value"))
                ),
                "std_vf_value": torch.mean(
                    torch.stack(self.get_tower_stats("std_vf_value"))
                ),
            }
        )
